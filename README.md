...
3. Detalhes e desafios do projeto
    ...
    .Em registros que são apenas resultados de cálculos feitos após o usuário informar um mês e ano(por exemplo, 'Saldo mensal', 'Condomínio (radio buttom 'Condomínio (membros)')'), só há a opção de           deletar os registros. Quando envolve registros em bloco (por exemplo, 'Condomínio (radio buttom 'Condomínio (membros)')'), A remoção dos registros também é feita em bloco, pelo mês e ano informados        pelo usuário
